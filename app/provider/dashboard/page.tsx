  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const profileResponse = await fetch(`http://localhost:5000/api/provider/profile/${providerId}`);
        if (!profileResponse.ok) throw new Error('Failed to fetch provider data');
        const profileData = await profileResponse.json();
        setProviderData(profileData);

        // Fetch recent bookings
        const bookingsResponse = await fetch(`http://localhost:5000/api/booking/provider/${providerId}/bookings`);
